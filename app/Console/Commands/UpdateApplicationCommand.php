...
class UpdateApplicationCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'Mohit:update';
    ...
    {
        $this->line('<bg=blue>Welcome to the Mohit update wizard</>');
        ...
        if (posix_getuid() == 0) {
            $this->error('This Command cannot be run by root user, thank you for using Mohit');
            exit;
        }

        if (!$this->confirm('Do you wish to continue?')) {
            $this->error('Operation cancelled, thank you for using Mohit');
            exit;
        }
    }
